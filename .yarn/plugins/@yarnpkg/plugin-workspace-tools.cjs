      ...
      var Ie = X(require("@yarnpkg/cli")), ge = X(require("@yarnpkg/core")), Ee = X(require("@yarnpkg/core")),
        ...
        xe = class extends Ie.BaseCommand {
          ...
          async execute() {
            ...
            let k = this.parallel ? this.jobs === "unlimited" ? Infinity : Number(this.jobs) || Math.max(1, (0, yr.cpus)().length / 2) : 1,
              ...
              ne = await Ee.StreamReport.start({ configuration: t, stdout: this.context.stdout }, async E => {
                let b = async (C, { commandIndex: M }) => {
                  ...
                  let l = zn(C, {
                    ...
                    commandIndex: M,
                  }), [H, w] = _r(E, { prefix: l, interlaced: R }), [j, c] = _r(E, { prefix: l, interlaced: R });
                  try {
                    this.verbose && E.reportInfo(null, `${l} Process started`);
                    let u = Date.now(), I = await this.cli.run([this.commandName, ...this.args], {
                      cwd: C.cwd,
                      stdout: H,
                      stderr: j,
                    }) || 0;
